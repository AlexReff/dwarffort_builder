...
export function tokenize(str: string, maxWidth: number) {
    ...
    str.replace(RE_COLORS, (match, type, name, index) => {
        /* string before */
        const thisPart = str.substring(offset, index);
        if (thisPart.length) {
            result.push({
                type: TYPE_TEXT,
                value: thisPart,
...
function breakLines(tokens: any[], maxWidth: number) {
    ...
    while (i < tokens.length) { /* take all text tokens, remove space, apply linebreaks */
        ...
        if (lineLength + token.value.length > maxWidth) { /* line too long, find a suitable breaking spot */
            ...
            let thisIdx = -1;
            while (1) {
                const nextIndex = token.value.indexOf(" ", thisIdx + 1);
                if (nextIndex === -1) { break; }
                if (lineLength + nextIndex > maxWidth) { break; }
                thisIdx = nextIndex;
            }

            if (thisIdx !== -1) { /* break at space within this one */
                token.value = breakInsideToken(tokens, i, thisIdx, true);
            } else if (lastTokenWithSpace !== -1) { /* is there a previous token where a break can occur? */
                const thisToken = tokens[lastTokenWithSpace];
                const breakIndex = thisToken.value.lastIndexOf(" ");
                thisToken.value = breakInsideToken(tokens, lastTokenWithSpace, breakIndex, true);
